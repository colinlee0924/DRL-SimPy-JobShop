...
def test(args, _env, agent, writer, CHECK_PTS):
    ...
    print(df)
    df.to_csv('paper_testing_result_09_4m200n.csv')
...
def main():
    ...
    writer = SummaryWriter(f'test_log/paper-DDQN-attention-h4-load09-{MODEL_VERSION}')
    ...
    agent.load(f'model/djss_attention/paper/ddqn-attention-4m300-h4-stateNormalized-{MODEL_VERSION}.pth-ck-{CHECK_PTS}.pth')
    # agent.load(f'model/djss_attention/paper/ddqn-attention-4m300-h4-stateNormalized-{MODEL_VERSION}.pth')
