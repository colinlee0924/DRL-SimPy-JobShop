...
class Source:
    ...
    def _generate_order(self):
        ...
        # while True:
            ...
            num_op    = np.random.randint(self.fac.low_op, 
                                          self.fac.high_op)
            routing   = np.arange(self.fac.num_machine)
            prc_time  = [np.random.uniform(low=self.fac.low_proc, 
                                           high=self.fac.high_proc) for _ in routing]
...
#################
    ...
    def dispatch_for(self, machine):
        ...
        if not mulit_jobs_in_queue or self.fac.throughput < self.fac.warmup_job:
    ...
    def order_arrive(self, *orders):
        ...
        for order in orders:
            ...
            if order.id < self.fac.num_job:
                # self.fac.tb_proc_status[order.id][order.progress] = 0
...
class Factory:
    def __init__(self, jssp_config, default_rule, util, log=False):
        ...
        self.num_machine  = 6#6 #jssp_config.num_machine
        self.num_op       = 4 # 4 #3#4#6
        ...
        self.high_proc = 8
        self.low_op  = 2
        self.high_op = self.num_machine + 1
    ...
    def build(self):
        ...
        ###############################################################
        # self.tb_machine_no  = np.ones((self.num_job, self.num_machine)) * -1
        # self.tb_proc_times  = np.ones((self.num_job, self.num_machine)) * -1
        # self.tb_proc_status = np.ones((self.num_job, self.num_machine)) * -1
        ###############################################################
...
#################
    def _get_state(self):
        ...
        self.observations[0] = (self.tb_machine_no.copy() + 1 )/ self.num_machine
        # self.observations[0] = self.tb_machine_no.copy() #/ (self.num_machine - 1)
        # self.observations[1] = self.tb_proc_times.copy()
...
#################
    def _get_reward(self):
        ...
        waste_time           = avg_queue_time * 0.7 + avg_idle_time * 0.3
        ...
        if self.terminal.triggered:
            ...
            if compare <= 1:
                credit, compare = 500, 1
            return (last_waste_time - waste_time) + credit/compare
        return (last_waste_time - waste_time)

    def _get_reward_makespan(self):
        ...
        # final state
        if self.terminal:
            makespan = self.env.now #self.makespan
            # optimal  = self.opt_makespan
            optimal  = 600
            if makespan == optimal:
                reward += 500
            else:
                reward += 1000 / (makespan - optimal)
...
#################
    def step(self, action):
        ...
        # reward     = self._get_reward_makespan()
    ...
    for util in lst_util:
        ...
        # for rule in range(1):
            ...
            # for rep in range(1):
                ...
                [util, rule, total_reward, makespan, mean_flow_time, avg_utils]
                counter += 1
        ...
        # df_result.to_csv("noWarmup_200n_result_85_90_95_m4.csv")
    df_result.to_excel("zWarmup_200n_result_85_90_95_m6.xlsx")
    print(df_result)
